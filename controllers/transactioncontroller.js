...
const transactionlist = async (req, res) => {
    try {
        ...
        const [[{ count }]] = await connection.promise().query('SELECT COUNT(*) AS count FROM Transactions');
        const totalPages = Math.ceil(count / itemsPerPage); 

        const offset = (currentPage - 1) * itemsPerPage;

        const [transactions] = await connection.promise().query(`SELECT * FROM Transactions LIMIT ?, ?`, [offset, itemsPerPage]);
...
const AddTransaction = async (req, res) => {
    try {
        const [customers] = await connection.promise().query('SELECT * FROM Customers');
        ...
        const [paymentTypes] = await connection.promise().query('SELECT * FROM Payments');

        res.render('AddTransaction', { customers, groupedCategories, paymentTypes ,categoryLength});
    } catch (error) {
...
const insertTransaction = async (req, res) => {
    ...
    try {
        await connection.promise().query(query, [transaction_date, customer_id, amount, category_name, subcategory_name, payment_method, notes, margin]);
        res.json({ success: true });
    } catch (error) {
        console.error('Error inserting transaction:', error);
        res.status(500).json({ success: false, message: 'An error occurred while inserting the transaction.' });
    }
};
...
const edittransaction = async (req, res) => {
    try {
        ...
        const [subcategories] = await connection.promise().query(
...
const updatetransaction = async (req, res) => {
    ...
    try {
        ...
        return res.status(500).json({ success: false, message: 'An error occurred while updating the transaction.' });
    }
};
...
const deletetransaction = async (req,res)=>{
    try {
        ...
        await connection.promise().query(`DELETE FROM Transactions  WHERE id = ?`, [id]);
...
const downloadtransaction = async (req, res) => {
    try {
        ...
        doc
            .font('Helvetica')
            .fontSize(12)
            .text(`Transaction Date: ${new Date(transaction.transaction_date).toDateString()}`, { align: 'left' })
            .moveDown(0.5);
        ...
        doc
            .text(`Amount: $${parseFloat(transaction.amount).toFixed(2)}`, { align: 'left' }) // Changed line
            .moveDown(0.5);
        ...
        doc
            .text(`Margin: $${transaction.margin}`, { align: 'left' })
            .moveDown(0.5);
        
        if (transaction.notes) {
            doc
                .text(`Notes: ${transaction.notes}`, { align: 'left' })
                .moveDown(0.5);
        }
